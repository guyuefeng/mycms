...
namespace Mycms.Web.UI
{
    public partial class BasePage : System.Web.UI.Page
    {
        protected internal Mycms.Model.siteconfig config = new BLL.siteconfig().loadConfig(Utils.GetXmlMapPath(DTKeys.FILE_SITE_XML_CONFING), true);
        protected internal Mycms.Model.userconfig uconfig = new BLL.userconfig().loadConfig(Utils.GetXmlMapPath(DTKeys.FILE_USER_XML_CONFING));
        /// <summary>
        /// 父类的构造函数
        /// </summary>
        public BasePage()
        ...
        public bool IsUserLogin()
        {
            ...
            {
                //检查Cookies
                string username = Utils.GetCookie(DTKeys.COOKIE_USER_NAME_REMEMBER, "Mycms"); //解密用户名
                string password = Utils.GetCookie(DTKeys.COOKIE_USER_PWD_REMEMBER, "Mycms");
                if (username != "" && password != "")
                {
                    BLL.users bll = new BLL.users();
                    Mycms.Model.users model = bll.GetModel(username, password, 0);
                    if (model != null)
                    {
        ...
        /// 取得用户信息
        /// </summary>
        public Mycms.Model.users GetUserInfo()
        {
            if (IsUserLogin())
            {
                Mycms.Model.users model = HttpContext.Current.Session[DTKeys.SESSION_USER_INFO] as Model.users;
                if (model != null)
                {
